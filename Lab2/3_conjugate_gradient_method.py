...
def searchLambda(x, function, antigrad, p, e):
    y = lambda l: function(x + l * p)
    return brent(y, e)

def searchB(last, now, antigrad):
    return (now**2 + antigrad(now)**2) / (last**2 + antigrad(last)**2)
...
def run(function, antigrad):
    ...
    while abs(last - now) > e:
        l = searchLambda)(x, function, antigrad, p, e)
        temp = now
        now = now + l * p
        last = temp
        b = searchB(last, now, antigrad)
        p = antigrad(now) + b * p
